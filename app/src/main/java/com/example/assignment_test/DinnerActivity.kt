...
class DinnerActivity:AppCompatActivity() {
    ...
    private val breakfastRef = database.getReference("dinner")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        linearLayout = findViewById(R.id.foodLayout_dinner)
        foodList = mutableListOf()

        val foodRef = FirebaseDatabase.getInstance().getReference("dinner")
//            val newFoodRef = breakfastRef.push()
//
//            val food = FoodItem("Milk", 100)
//            newFoodRef.setValue(food)

//        val foodItem = FoodItem(name = "Grilled salmon", calories = 240)
...
//        }
        ...
        foodRef.addValueEventListener(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@DinnerActivity, error.message, Toast.LENGTH_SHORT).show()
            }
        })
    }
        ...
        deleteIcon.setOnClickListener {
            showConfirmDialog(food, foodView) { confirmed ->
                if (confirmed as Boolean) {
                    ...
                    val foodsRef = database.getReference("dinner")
    ...
    companion object {
        private const val TAG = "DinnerActivity"
    }
}